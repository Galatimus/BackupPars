...
while True:
     ...
     class Rosreal_Com(Spider):
	  ...
	  def task_item(self, grab, task):
	       ...
	       try:
		    ray = grab.doc.select(u'//p[@class="pbig_gray"]/b/a[contains(@title,"Недвижимость в")][contains(text(),"район")]').text()
		  #print ray 
	       except DataNotFound:
		    ray = ''          
	       try:
		    if  grab.doc.select(u'//p[@class="pbig_gray"]/b/a[contains(@title,"Недвижимость в")][contains(text(),"район")]').exists()==False:
			 ...
			 punkt= grab.doc.select(u'//p[@class="pbig_gray"][1]/b/a[3]').text()
	       except IndexError:
		    punkt = ''
		    
	       try:
		    ter = grab.doc.select(u'//p[@class="pbig_gray"][2]/a').text()
	       except IndexError:
		    ter =''
	       ...
	       try:
		    naz = grab.doc.select(u'//p[@class="pbig_gray"][3]').text().split(', ')[0]
	       except IndexError:
		    naz = ''
	       ...
	       try:
		    ...
		    price = ''
	       try:
		    cena_za = grab.doc.select(u'//b[@class="red"]/preceding-sibling::b[1]').text().replace(u'Цена за ','').replace(u'Стоимость аренды ','').replace(u'кв.м.',u'м2').replace(':','') 
	       except IndexError:
		    cena_za = ''	       
	       
		    
	       try:
		    plosh = grab.doc.select(u'//p[@class="pbig_gray"][3]').text().split(', ')[1]
	       except IndexError:
		    plosh = ''
		    
	       
	       try: 
		    klass = grab.doc.select(u'//p[@class="pbig_gray"][contains(text(),"-класс")]').text()
	       except DataNotFound:
		    klass =''
	       ...
	       try:
		    ohrana = grab.doc.select(u'//div[@class="section_right"]/p[1]').text()
	       except DataNotFound:
		    ohrana =''
	       try:
	       ...
	       try:
		    opis = grab.doc.select(u'//div[@class="section_right"]/following-sibling::div[1]').text().split(u'ready')[0].replace(u'ymaps.','')  
	       except IndexError:
		    opis = ''
