...
time.sleep(2)
os.system('echo %s|sudo -S %s' % ('1122', 'mongo actualcom2 --eval "db.dropDatabase()"'))
...
class Gis(Spider):
       ...
       def prepare(self):
              ...
              self.workbook = xlsxwriter.Workbook(u'com/ActCom2.xlsx')#+datetime.today().strftime('%d.%m.%Y')+'.xlsx')
       ...
       def task_generator(self):
              for ak in range(1,self.sheet.nrows):
                     ...
                     yield Task ('post',url= links,ist=ist,cod=cod,network_try_count=20)
       ...
       def task_post(self,grab,task):
              ...
              #if 'IRR' in opcs:
              elif 'из рук в руки' in task.url:
...
bot = Gis(thread_number=50, network_try_limit=200)
bot.setup_queue('mongo', database='actualcom2',host='127.0.0.1')
