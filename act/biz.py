...
time.sleep(2)
os.system('echo %s|sudo -S %s' % ('1122', 'mongo actualbiz --eval "db.dropDatabase()"'))
...
class Actuall(Spider):
       ...
       def prepare(self):
              ...
              self.workbook = xlsxwriter.Workbook(u'biz/ActBiz.xlsx')#+datetime.today().strftime('%d.%m.%Y')+'.xlsx')
       ...
       def task_generator(self):
              for ak in range(1,self.sheet.nrows):
                     ...
                     yield Task ('post',url= links,ist=ist,cod=cod,network_try_count=5)
       ...
       def task_post(self,grab,task):
              ...
              if 'avito.ru' in task.ist:
                     ...
                     if grab.doc.select(u'//div[@class="b-seller-phone"]').exists() == True:
                            akt = 'True'
                     else:
                            akt = 'False'                     
              ...
              elif 'bizzona.ru' in task.ist:
                     if grab.doc.code ==200:
                            akt = 'True'
                     else:
                            akt = 'False'
       
              elif 'business_asset_exchange' in task.ist:
                     if grab.doc.code ==200:
                            akt = 'True'
                     else:
                            akt = 'False'
       
              elif 'businessesforsale' in task.url:
                     if grab.doc.code ==200:
                            akt = 'True'
                     else:
                            akt = 'False'                        
       
              elif 'buy business portal' in task.ist:
                     if grab.doc.code ==200:
                            akt = 'True'
                     else:
                            akt = 'False'
       
              elif 'delomart.ru' in task.ist:
                     if grab.doc.code ==200:
                            akt = 'True'
                     else:
                            akt = 'False'
              ...
              elif 'альтера инвест' in task.ist:
                     if grab.doc.code ==200:
                            akt = 'True'
                     else:
                            akt = 'False'
              ...
              elif 'bizmast.ru/' in task.url:
                     if grab.doc.code ==200:
                            akt = 'True'
                     else:
                            akt = 'False'
       
              elif 'рбк бизторг' in task.ist:
                     if grab.doc.code ==200:
                            akt = 'True'
                     else:
                            akt = 'False'
...
bot = Actuall(thread_number=50, network_try_limit=50)
bot.setup_queue('mongo', database='actualbiz',host='127.0.0.1')
