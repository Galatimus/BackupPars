...
workbook = xlsxwriter.Workbook(u'zemm/0001-0064_00_У_001-0022_DOMV45.xlsx')
...
class Brsn_Zem(Spider):
     ...
     def task_generator(self):
	  for x in range(1,10):
               yield Task ('post',url='http://domovoy45.ru/uchastki?city_id=3&optype=1&page=%d'%x,network_try_count=100)
                 
     def task_post(self,grab,task):
    
	  for elem in grab.doc.select(u'//div[@class="rg-img"]/a'):
     ...
     def task_item(self, grab, task):
	  try:
	       sub = u'Курганская область'#grab.doc.select(u'//label[contains(text(),"Адрес:")]/following-sibling::p').text().split(', ')[0]
	  except IndexError:
	       sub = ''
	  try:
	     ...
	     #print ray 
	       ...
	       ray = ''          
	  try:
	       punkt= u'Курган'#grab.doc.select(u'//span[@class="hp_caption"][contains(text(),"Город:")]/following-sibling::text()').text()
	  except IndexError:
	       punkt = ''
	       
	  try:
	       ter= grab.doc.select(u'//label[contains(text(),"Адрес:")]/following-sibling::span').text()
	  except IndexError:
	       ter =''
	       
	  try:
	       
	       uliza = grab.doc.select(u'//label[contains(text(),"Улица:")]/following-sibling::span[contains(text()," ул.")]').text()#.split(u' /')[0]
	       #else:
		    #uliza = ''
	  except IndexError:
	       uliza = ''
	  ...
	  try:
	       ...
	       plosh = ''
	  try:
	       vid = grab.doc.select(u'//h3').text().split(u', ')[1]
	  except DataNotFound:
	       vid = '' 
	  ...
	  try:
	       opis = grab.doc.select(u'//div[@class="row-fluid"]/p').text() 
	  except DataNotFound:
	       opis = ''
	  ...
	  try:
	       comp = grab.doc.select(u'//div[@class="avtor"]/a[contains(@href, "firma")]/h4').text()
	  except IndexError:
	       comp = ''
     ...
     def task_write(self,grab,task):
	  ...
	  self.ws.write(self.result, 23, u'Домовой45')
...
print('Done!')
time.sleep(5)
os.system("/home/oleg/pars/small/dmvoy_com.py")
