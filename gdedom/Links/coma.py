...
while True:
     ...
     class Gdedom_Zem(Spider):
	  def prepare(self):
	       ...
	       self.workbook = xlsxwriter.Workbook(u'com/Gdeetotdom_%s' % bot.sub + u'_Коммерческая_'+oper+str(i+1)+'.xlsx')
	       self.ws = self.workbook.add_worksheet(u'Gdeetotdom_Коммерческая')
     ...
     bot = Gdedom_Zem(thread_number=5,network_try_limit=1000)
