...
i = 0
l= open('links/zem_a.txt').read().splitlines()
dc = len(l)
page = l[i]
...
while True:
     ...
     class Kuz_zap(Spider):
	  ...
	  def task_item(self, grab, task):
	       try:
	            mesto =  grab.doc.select(u'//div[contains(text(),"Адрес")]/following-sibling::div').text()
	       except IndexError:
		    mesto = ''
	       ...
	       try:
		    conv = [ (u'сегодня', (datetime.today().strftime('%d.%m.%Y'))),
	                     ...
	                     (u'июня', '.06.2017'),(u'июля', '.07.2017'),(u'августа', '.08.2017'),(u'января', '.01.2017'),(u'февраля', '.02.2017'),
		             (u'марта', '.03.2017'),(u'апреля', '.04.2017'),(u'мая', '.05.2017'),
	                     (u'ноября', '.11.2017'),(u'сентября', '.09.2017'),(u'октября', '.10.2017'),(u'декабря', '.12.2016')]
     ...
     bot = Kuz_zap(thread_number=2, network_try_limit=1000)
