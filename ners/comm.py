     ...
     class QP_Com(Spider):
	  def prepare(self):
	       ...
	       for p in range(1,51):
		    ...
		    self.pag = 0	       
	       self.workbook = xlsxwriter.Workbook(u'com/Ners_'+oper+str(i+1)+'.xlsx')
	  ...
	  def task_generator(self):
	       for x in range(self.pag+1):
                    yield Task ('post',url=self.f+'?start='+str(x*10),refresh_cache=True,network_try_count=100)
	  
	  
	  
	  def task_page(self,grab,task):
	       try:         
		    pg = grab.doc.select(u'//div[@class="pagination"]/span/strong/following-sibling::a[1]')
