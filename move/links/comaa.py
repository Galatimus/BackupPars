...
class move_Com(Spider):
     def prepare(self):
	  self.workbook = xlsxwriter.Workbook(u'com/Move_aa.xlsx')
     ...
     def task_generator(self):
	  yield Task ('next',url='https://move.ru/geo/',refresh_cache=True,network_try_count=100)
	  
	  
	  
     def task_next(self,grab,task):
	  for el in grab.doc.select(u'//li[@class="geo-page__list-item"]/a[1]'):
	       urr = grab.make_url_absolute(el.attr('href'))  
	       #print urr
	       yield Task('post', url=urr+'/arenda_kommercheskoy_nedvizhimosti/',refresh_cache=True,network_try_count=100)
     ...
     def task_page(self,grab,task):
	  try:
	       pg = grab.doc.select(u'//a[@class="pagination-block__paginate-next pagination-block__ctrl-title"][contains(text(),"следующая")]')
...
bot = move_Com(thread_number=7, network_try_limit=2000)
