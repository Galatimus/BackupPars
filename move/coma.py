...
while True:
     ...
     class move_Com(Spider):
	  def prepare(self):
	       ...
	       for p in range(1,51):
		    try:
			 ...
			 g.go(self.f+'/arenda_kommercheskoy_nedvizhimosti/')
	       ...
	       self.workbook = xlsxwriter.Workbook(u'com/Move_Comm_'+oper+'_'+str(i+1)+'.xlsx')
	  ...
	  def task_generator(self):
	       for x in range(1,self.pag+1):
                    yield Task ('post',url=self.f+'/arenda_kommercheskoy_nedvizhimosti/?page=%d'%x,refresh_cache=True,network_try_count=100)
     ...
     bot = move_Com(thread_number=15, network_try_limit=1000)
