...
workbook = xlsxwriter.Workbook(u'0001-0002_00_У_001-0284_MULIST.xlsx')
...
class Farpost_Com(Spider):
     ...
     def task_generator(self):
	  for x in range(1,40000):
	       yield Task ('post',url='https://multilisting.su/?advertisement[property_type][]=2&page=%d'%x,refresh_cache=True,network_try_count=100)
     ...
     def task_item(self, grab, task):
	  ...
	  try:
	       udal = grab.doc.select(u'//li[contains(text(),"КП")]').text().split(': ')[1]
	  except IndexError:
	       udal = ''
          try:
     ...
     def task_write(self,grab,task):
	  if task.proj['sub'] <> '':
	       ...
	       self.ws.write(self.result, 31, task.proj['sub']+', '+task.project['rayon'])
	       ...
	       self.ws.write(self.result, 25, task.project['segment'])
	       self.ws.write(self.result, 9, task.project['trassa'])
	       self.ws.write(self.result, 6, task.project['udal'])
	       ...
	       self.ws.write(self.result, 23, u'Электронная база недвижимости')
	       ...
	       self.ws.write(self.result, 9, task.project['operazia'])
	       print('*'*50)
	       #print task.sub
