...
class Torgi_Zem(Spider):
     ...
     def prepare(self):
	  ...
	  self.ws.write(0, 54, u"Минимальная цена")
	  self.ws.write(0, 55, u"Ежемесячный платеж")
	  self.ws.write(0, 56, u"Номер лота")
     ...
     def task_item(self, grab, task):
	  ...
	  try:
          ...
          try:
	       ...
	       min_cena = ''
	  try:
	       plat =  grab.doc.select(u'//label[contains(text(),"Ежемесячный платеж")]/following::span[1]').text()
          except IndexError:
	       plat = ''	  
	  
	  try:     
	       lot = grab.doc.select(u'//title').text().split(':')[0]
	  except IndexError:
	       lot = ''
	
	  #g2 = grab.clone()
	  #g2.request(url='https://torgi.gov.ru/?wicket:interface=:1:notificationEditPanel:tabs:tabs-container-parent:tabs-container:tabs:1:link::IBehaviorListener:0:2')
	  #time.sleep(1)
	  #print g2.doc.select(u'//td[@class="datacell center"]/span/span[1]').text()
	  #del g2
	  
	    #grr.setup(url='https://torgi.gov.ru/?wicket:interface=:1:notificationEditPanel:tabs:tabs-container-parent:tabs-container:tabs:1:link::IBehaviorListener:0:2')
		    #yield Task('lot',grab=gr,project=projects,refresh_cache=True,network_try_count=100)
		    #del grr        
     ...
     def task_write(self,grab,task):
	  ...
	  print  task.project['platech']
	  print  task.project['lot']
	  #############################
	  print('*'*50)
	  ...
	  self.ws.write(self.result, 54, task.project['cenam'])
	  self.ws.write(self.result, 55, task.project['platech'])
	  self.ws.write(self.result, 56, task.project['lot'])
