     ...
     class Gdedom_Zem(Spider):
	  def prepare(self):
	       ...
	       for p in range(1,51):
			 ...
			 g.proxylist.load_file(path='../ivan.txt',proxy_type='http') 
                         g.go(self.f)
			 self.sub = g.doc.select(u'//div[@class="header-location"]/a').text()#.split(u' в ')[1].replace(u'регионе ','').replace(u'Подмосковье',u'Московская область')
			 print self.sub
			 del g
			 break
	       ...
	       self.workbook = xlsxwriter.Workbook(u'com/Sibdom_Comm_'+str(i+1)+'.xlsx')
	  ...
	  def task_generator(self):
          ...
          def task_goto(self,grab,task):
	       for li in grab.doc.select(u'//div[@class="tab-content"]/div/ul/li/a[contains(@href,"commercial")]'):
	  ...
	  def task_post(self,grab,task):
	       for elem in grab.doc.select(u'//div[@class="catalog-product-list catalog-product-list--commercials"]/a'):
	  ...
	  def task_item(self, grab, task):
	       ...
	       try:
		    udal = grab.doc.select(u'//div[@class="page-container"]/div[3]/a/text()[1]').text()
	       except IndexError:
		    udal = ''
	       ...
	       try:
		    plosh = grab.doc.select(u'//span[contains(text(),"Общая площадь")]/following::div[1]').text()
	       except IndexError:
		    plosh = ''

	       try:
		    vid = grab.doc.select(u'//span[contains(text(),"Этаж")]/following::div[1]').text()
	       except IndexError:
		    vid = '' 
	       ...
	       try:
		    ohrana = grab.doc.select(u'//span[contains(text(),"Год постройки")]/following::div[1]').text()
	       except IndexError:
		    ohrana =''
	       try:
	       ...
	       try:
		    elek = re.sub('[^\d\.\ ]','',grab.doc.rex_text(u'POINT(.*?)selection').split(' ')[0])
	       except IndexError:
		    elek =''
		    
	       try:
		    lng = re.sub('[^\d\.\ ]','',grab.doc.rex_text(u'POINT(.*?)selection').split(' ')[1])
	       except IndexError:
	            lng =''		    
	       try:
		    teplo = grab.doc.select(u'//ul[@class="breadcrumbs-list"]/li[3]/a').text().split(u'добавлено ')[1]
	       except IndexError:
		    teplo =''
	       ...
	       try:
                    park = grab.doc.select(u'//ul[@class="breadcrumbs-list"]/li[3]/a').text().split(u'обновлено ')[1] 
	       except IndexError:
		    park = ''		    
	  ...
	  def task_write(self,grab,task):
	       ...
	       self.ws.write(self.result, 28, task.project['opera'])
	       ...
	       self.ws.write(self.result, 30, task.project['parkov'])
	       ...
	       self.ws.write(self.result, 19, u'Сетевое издание «Сибдом»')
